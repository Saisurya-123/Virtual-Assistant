...
def virtual_assistant(data):
    ...
    if "email" in data:
        ...
        to=listen().lower()
        edict={'Saisurya':'Saisurya'} #give mail ids
        toaddr = edict[to]
        ...
        mail.starttls()
        #login
        mail.login('saisuryarajanala63@gmail.com','ypfq xpzq xwax cquh')
        #enter mailid and app password make sure you enable less secure app access
        mail.sendmail('saisuryarajanala63@gmail.com',toaddr,content)
        mail.close()
        respond('Email Sent')
    if "stop" in data:
        listening=False
